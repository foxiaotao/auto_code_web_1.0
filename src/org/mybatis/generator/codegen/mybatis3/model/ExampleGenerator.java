...
public class ExampleGenerator extends AbstractJavaGenerator {
    ...
    public List<CompilationUnit> getCompilationUnits() {
        ...
        //add field, getter, setter for join添加关联查询的条件
        Field field = new Field();
        field.setVisibility(JavaVisibility.PROTECTED);
        field.setType(FullyQualifiedJavaType.getStringInstance());
        field.setName("join"); //$NON-NLS-1$
        commentGenerator.addFieldComment(field, introspectedTable);
        topLevelClass.addField(field);
        
        method = new Method();
        method.setVisibility(JavaVisibility.PUBLIC);
        method.setName("setJoin"); //$NON-NLS-1$
        method.addParameter(new Parameter(FullyQualifiedJavaType
                .getStringInstance(), "join")); //$NON-NLS-1$
        method.addBodyLine("this.join = join;"); //$NON-NLS-1$
        commentGenerator.addGeneralMethodComment(method, introspectedTable);
        topLevelClass.addMethod(method);
        ...
        method.setName("getJoin"); //$NON-NLS-1$
        method.addBodyLine("return join;"); //$NON-NLS-1$
        commentGenerator.addGeneralMethodComment(method, introspectedTable);
        topLevelClass.addMethod(method);
    ...
    private InnerClass getGeneratedCriteriaInnerClass(
        ...
        method.setVisibility(JavaVisibility.PUBLIC);
        method.setReturnType(FullyQualifiedJavaType.getCriteriaInstance());
        method.setName("andJoinWhere"); //$NON-NLS-1$
        method.addParameter(new Parameter(FullyQualifiedJavaType
                .getStringInstance(), "whereSql")); //$NON-NLS-1$
        method.addBodyLine("addCriterion(whereSql);");
        ...
        for (IntrospectedColumn introspectedColumn : introspectedTable
            ...
            answer.addMethod(getSetInOrNotInMethodBySql(introspectedColumn, true));
            answer.addMethod(getSetInOrNotInMethodBySql(introspectedColumn, false));
    ...
    private Method getSingleValueMethod(IntrospectedColumn introspectedColumn,
        ...
        sb.append(introspectedTable.getAliasedFullyQualifiedTableNameAtRuntime()+"."+MyBatis3FormattingUtilities
    ...
    /**
     * 生成in 或 not in （sql），模式的语句段
     * @param introspectedColumn
     * @param inMethod
     * @return
     */
    private Method getSetInOrNotInMethodBySql(IntrospectedColumn introspectedColumn,
            boolean inMethod) {
        Method method = new Method();
        method.setVisibility(JavaVisibility.PUBLIC);

        method.addParameter(new Parameter(FullyQualifiedJavaType.getStringInstance(), "sql")); //$NON-NLS-1$
        StringBuilder sb = new StringBuilder();
        sb.append(introspectedColumn.getJavaProperty());
        sb.setCharAt(0, Character.toUpperCase(sb.charAt(0)));
        sb.insert(0, "and"); //$NON-NLS-1$
