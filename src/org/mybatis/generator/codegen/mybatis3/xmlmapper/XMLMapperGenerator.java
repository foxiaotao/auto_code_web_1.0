...
import org.mybatis.generator.codegen.mybatis3.xmlmapper.elements.CountAllElementGenerator;
...
import org.mybatis.generator.codegen.mybatis3.xmlmapper.elements.ExampleWhereClauseElementPageGenerator;
...
import org.mybatis.generator.codegen.mybatis3.xmlmapper.elements.SelectByExamplePageWithoutBLOBsElementGenerator;
...
public class XMLMapperGenerator extends AbstractXmlGenerator {
    ...
    protected XmlElement getSqlMapElement() {
        ...
        addExampleWhereClausePageElement(answer);
        ...
        addSelectByExamplePageWithoutBLOBsElement(answer);
	...
	protected void addSelectByExamplePageWithoutBLOBsElement(XmlElement parentElement) {
		AbstractXmlElementGenerator elementGenerator = new SelectByExamplePageWithoutBLOBsElementGenerator();
    ...
    protected void addExampleWhereClausePageElement(XmlElement parentElement) {
    	if (introspectedTable.getRules()
                .generateMyBatis3UpdateByExampleWhereClause()) {
            AbstractXmlElementGenerator elementGenerator = new ExampleWhereClauseElementPageGenerator();
            initializeAndExecuteGenerator(elementGenerator, parentElement);
        }
    ...
    protected void addBatchInsertElement(XmlElement parentElement) {
    	if(introspectedTable.getGeneratedKey()!=null){//如果没有配置GeneratedKey，那么就不做该方法处理
	        ...
	        initializeAndExecuteGenerator(elementGenerator, parentElement);
    	}else if(context.getJdbcConnectionConfiguration().getDriverClass().toLowerCase().contains("mysql")){
    		AbstractXmlElementGenerator elementGenerator = new BatchInsertElementGenerator(false);
	        initializeAndExecuteGenerator(elementGenerator, parentElement);
    	}
    }
    ...
    protected void addCountALlElement(XmlElement parentElement) {
    	if (introspectedTable.getRules().generateCountByExample()) {
